...
getModBoardNames uid role = case role of
  ...
  ModeratorRole -> do
    ...
    return [ boardName (entityVal b) | b@(Entity bid _) <- boards
...
createBoardH auth req = do
  ...
  when (isJust mExisting) $
    throwBadRequest $ "Board /" <> cbrName req <> "/ already exists"
...
adminCreateUserH auth req = do
  ...
  when (isJust mExisting) $
    throwBadRequest $ "Username '" <> curUsername req <> "' is already taken"
